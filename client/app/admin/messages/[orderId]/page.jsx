...
export default function OrderChatPage() {
  ...
  // Mark messages as read when admin visits this chat page
  const markMessagesAsRead = async () => {
    try {
      await API.put(`/order-messages/${orderId}/read`);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (orderId && user) {
      ...
      // যদি রুটটি অ্যাডমিনের হয়, তবে পেজে ঢোকার সাথে সাথে মেসেজগুলো রিড হিসেবে মার্ক করে দেব
  ...
  }, [orderId, user, authLoading, pathname]);
  ...
  const chatPartner = isRouteAdmin
    ? orderInfo?.userId || { name: "Customer", profileImage: "" } // If admin route, show customer info
