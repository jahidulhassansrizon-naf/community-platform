    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-0 sm:px-4 py-0 sm:py-6 flex h-[calc(100vh-64px)]">
        <div className="bg-white w-full sm:rounded-2xl shadow-sm border border-slate-200 flex overflow-hidden">
          {/* Sidebar */}
          <div
            className={`${showMobileChat ? "hidden sm:flex" : "flex"} w-full sm:w-80 border-r border-slate-100 flex-col`}
          >
              ...
              {conversations.map((conv) => {
                ...
                return (
                  <div
                    ...
                    className={`p-4 cursor-pointer hover:bg-slate-50 transition flex items-center gap-3 ${activeChat?._id === conv._id ? "bg-emerald-50/50" : ""}`}
                  >
                    <img
          ...
          {/* Chat Panel */}
          <div
            className={`${showMobileChat ? "flex" : "hidden sm:flex"} flex-1 flex-col bg-white`}
          >
            {activeChat ? (
              <>
                ...
                <div className="flex-1 p-6 overflow-y-auto space-y-4 bg-slate-50/50">
                  {messages.map((msg, i) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm ${isMe ? "bg-emerald-600 text-white rounded-br-none" : "bg-white border border-slate-200 rounded-bl-none shadow-sm text-slate-700"}`}
                        >
                          {msg.content}
                        </div>
                      </div>
